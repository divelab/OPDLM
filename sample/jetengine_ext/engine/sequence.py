...
class Sequence:
    ...
    def commit_block(self, block_tokens: list[int]):
        ...
        #    Skip EOS checking for prompt tokens that ended up in the
        #    first intermediate block (prompt not aligned to block_size).
        final_block = []
        k = 0
        for token_id in block_tokens:
            is_prompt_token = (self.num_tokens + k) < self.num_prompt_tokens
            if not is_prompt_token and not self.ignore_eos and (token_id == self.eos_token_id or token_id in self.stop_words):
                final_block.append(token_id)
                k += 1
                self.status = SequenceStatus.FINISHED
                break
