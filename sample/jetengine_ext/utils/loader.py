...
def _load_expert_weight_to_fused(model: nn.Module, weight_name: str, weight_tensor: torch.Tensor, shard_id=None, tp_size: int | None = None, tp_rank: int | None = None):
...
def load_model(model: nn.Module, path: str, tp_size: int | None = None, tp_rank: int | None = None):
    """Load model weights from safetensors files.

    Args:
        tp_size: Explicit tensor-parallel world size. When running inside an
            existing distributed context (e.g. accelerate launch) the default
            ``dist.get_world_size()`` reflects the *training* world size, not
            the JetEngine tensor-parallel size. Pass ``tp_size=1, tp_rank=0``
            to load the full (unsharded) model on each rank.
        tp_rank: Explicit tensor-parallel rank.
    """
                ...
                for k in packed_modules_mapping:
                    if k in weight_name:
                        ...
                        if _is_moe_expert_weight(param_name):
                            _load_expert_weight_to_fused(model, param_name, f.get_tensor(weight_name), shard_id, tp_size=tp_size, tp_rank=tp_rank)
                        else:
                    ...
                    if _is_moe_expert_weight(weight_name):
                        _load_expert_weight_to_fused(model, weight_name, f.get_tensor(weight_name), tp_size=tp_size, tp_rank=tp_rank)
                    else:
